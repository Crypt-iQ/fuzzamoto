...
#[allow(dead_code)] // `Balanced` is retained as a selectable policy even though the default is `LateBiased`.
pub enum SnapshotPlacementPolicy {
    Balanced,
    /// Bias the snapshot toward the end of the program so more of the built-up state (e.g. a large
    /// mempool) is frozen into the snapshot and the fuzzed suffix is short.
    LateBiased,
}
...
pub struct IncrementalSnapshotStage<IS, S, OT> {
    ...
    /// Only take an incremental snapshot for inputs whose probed mempool has at least this many
    /// transactions; smaller inputs fall through to the root snapshot. Biases the (more expensive)
    /// incremental-snapshot machinery toward the large-mempool inputs it actually benefits.
    min_mempool_txs: usize,
...
impl<IS, S, OT> IncrementalSnapshotStage<IS, S, OT> {
    ...
    fn choose_position(&self, rand: &mut impl Rand, program_len: usize) -> Option<usize> {
        ...
        match self.policy {
            ...
            SnapshotPlacementPolicy::LateBiased => {
                if program_len == 1 {
                    return Some(0);
                }
                // Place the snapshot somewhere in the final quarter of the program, so the frozen
                // prefix captures nearly all the built-up state and only a short suffix is fuzzed.
                let start = program_len - (program_len / 4).max(1);
                let range = program_len - start;
                let nz_range = NonZeroUsize::new(range).expect("range should be non-zero");
                Some(start + rand.below(nz_range))
            }
        }
    }
}
    ...
    OT: libafl::observers::ObserversTuple<IrInput, S>,
{
    fn perform(
        ...
        // Bias toward large-mempool inputs: look up how many mempool txs this corpus entry
        // produced (recorded by the probe stage). Inputs below the threshold use the root snapshot.
        let mempool_txs = state
            .current_corpus_id()?
            .and_then(|id| {
                state
                    .metadata::<crate::stages::RuntimeMetadata>()
                    .ok()
                    .and_then(|rt| rt.mempool_tx_count(id))
            })
            .unwrap_or(0);

        if program_len == 0 || mempool_txs < self.min_mempool_txs || state.rand_mut().coinflip(0.04)
        {
            // Skip creating an incremental snapshot if we're using the empty program, the input
            // doesn't have enough mempool transactions to be worth it, or we randomly decide to
            // use the root.
            if self.inner_stage.should_restart(state)? {
                self.inner_stage.perform(fuzzer, executor, state, manager)?;
            }
