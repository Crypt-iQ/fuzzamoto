...
docker exec --workdir /fuzzamoto -e BITCOIND_PATH=/bitcoin/build_fuzz/bin/bitcoind $container_id cargo build --workspace --release --features fuzz
...
docker exec --workdir /fuzzamoto $container_id cp -L target/release/scenario_ir resources/scenario_ir
