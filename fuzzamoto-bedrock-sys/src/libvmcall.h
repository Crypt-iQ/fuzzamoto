 ...
 * id length cap). Mirrors the hypervisor's constants — keep in step with
 * FEEDBACK_BUFFER_MAX_PAGES in crates/bedrock-vmx/src/vm_state.rs, which
 * documents why the buffer is sized the way it is. */
#define VMCALL_FEEDBACK_BUFFER_MAX_SIZE (256U * 4096U)
#define VMCALL_FEEDBACK_BUFFER_ID_MAX_LEN 128U

/* Staging-buffer size for the file transfer hypercalls (file-fetch.c /
 * file-store.c).
 *
 * Deliberately its own constant rather than VMCALL_FEEDBACK_BUFFER_MAX_SIZE:
 * the transfers are chunked, so this only sets the chunk size, and sizing it
 * off the coverage cap coupled two unrelated things — raising the coverage cap
 * silently inflated every transfer buffer, and any guest built against a newer
 * header then failed registration against an older module (FB_ERR_BAD_SIZE).
 * 1 MB is the value both paths were effectively built and tested with. */
#define VMCALL_FILE_XFER_BUF_SIZE (256U * 4096U)
